...
$sql_total_quantity = "SELECT SUM(quantity) AS total_quantity FROM cart";
...
if (!empty($cart_items)) {
    $stmt_insert = $conn->prepare("INSERT INTO order_history (item, amount, status, request, quantity) VALUES (?, ?, 'Pending', 'Request', ?)");
    $stmt_delete = $conn->prepare("DELETE FROM cart WHERE id = ?");

    if ($stmt_insert && $stmt_delete) {
        $stmt_insert->bind_param("sdi", $item_name, $item_price, $item_quantity);
        ...
        foreach ($cart_items as $row) {
            ...
            if (!$stmt_insert->execute() || !$stmt_delete->execute()) {
                echo "Error processing cart items: " . $conn->error;
            }
        }
    } else {
...
<head>
    ...
    .cart-item-quantity, .cart-item-amount {
        flex: 1;
        text-align: right;
    }
...
<div class="container mt-3 rounded rounded-container">
                        ...
                        <div class="col-md-12">
                            ...
                            if (!empty($cart_items)) {
                                foreach ($cart_items as $row) {
                                    ...
                                    echo '<div class="cart-item-name">' . $row["name"] . '</div>';
                                    echo '<div class="cart-item-quantity">x' . $row["quantity"] . '</div>';
                                    echo '<div class="cart-item-amount">RM' . number_format($total_amount, 2) . '</div>';
                                    echo '</div>';
                                }
                            } else {
                            ...
                            ?>
                            <p class="shipping"><b>SHIPPING: RM<?php echo number_format($shipping, 2); ?></b></p>
                            <p class="subtotal"><b>SUBTOTAL: RM<?php echo number_format($subtotal, 2); ?></b></p>

                            <button type="submit" class="btn btn-custom btn-block" onclick="showSuccessPopup()">PROCEED TO PAYMENT</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div>
        ...
        <script>
            ...
            var receiptContent = document.querySelector('.card-body').innerHTML;
